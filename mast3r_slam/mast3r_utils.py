...
def vggt_match_symmetric(model,vggt, device, feat_i, pos_i, feat_j, pos_j, shape_i, shape_j,frame_i, frame_j,):
    ...
    #_,mast3r_C,_,mast3r_Q = mast3r_decode_symmetric_batch(model, feat_i, pos_i, feat_j, pos_j, shape_i, shape_j)
...
def showHistogram(tensors, tensor_names, bins=50):
    n = len(tensors)
...
def showHistogramNormalized(tensors, tensor_names, bins=50):
    n = len(tensors)
    ...
    for ax, tensor, name in zip(axes, tensors, tensor_names):
        ...
        data = t_norm.cpu().numpy().flatten()
        ax.hist(data, bins=bins, range=(0, 1))
        ax.set_title(f"{name} (min→0, max→1)")
...
def vggt_match_asymmetric(model, vggt, device,frame_i, frame_j, idx_i2j_init=None):
    ...
    _,mast3r_C,_,mast3r_Q = mast3r_asymmetric_inference(model, frame_i, frame_j)

    Q = C.clone()     ####################

    tensors = [C, mast3r_C, mast3r_Q]
    tensor_names = ["VGGT 's C", "Mast3r 's C", "Mast3r 's Q"]

    showHistogram(tensors, tensor_names, bins=50)

    #showHistogramNormalized(tensors, tensor_names, bins=50)
